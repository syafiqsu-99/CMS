      ...
      <table class="plc-table" ref="tableRef">
        <thead>
          <tr>
            ...
            <th v-for="sig in orderedSignals"
                :key="sig.address"
                :data-col="sig.address"
                :style="{ width: colWidths[sig.address] ? colWidths[sig.address] + 'px' : '90px', minWidth: '60px' }"
                class="col-signal"
                draggable="true"
                @dragstart="onDragStart($event, sig.address)"
                @dragover.prevent="onDragOver($event, sig.address)"
                @drop="onDrop($event, sig.address)"
                @dragend="onDragEnd">
              ...
              <div class="resize-handle"
                   @mousedown.prevent.stop="onResizeStart($event, sig.address)"></div>
            </th>
          </tr>
        </thead>
