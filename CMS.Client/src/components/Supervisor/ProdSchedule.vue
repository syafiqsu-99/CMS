  ...
  <div style="display: flex; flex-direction: column; overflow: hidden;">
    <v-toolbar flat density="compact" class="px-2">
      ...
      <div class="d-flex ga-2">
        <v-date-input v-model="production_date" label="Production Date" :max="maxDate" variant="outlined"
          density="compact" hide-details display-format="fullDate" style="width: 200px;" />
        <v-select label="Shift" v-model="shift" :items="[{ title: 'Morning', value: 1 }, { title: 'Night', value: 2 }]"
          density="compact" variant="outlined" hide-details style="width: 200px;" />
      </div>
    </v-toolbar>

    <div style="flex: 1; overflow-y: auto;">
      <v-data-table-virtual :headers="productHeaders" style="height: 67vh;" fixed-header :items="dailyReport"
        :loading="loading" loading-text="Loading" density="compact" class="elevation-1 fixed-table">
        <template v-slot:headers="{ columns }">
          <tr>
            <th v-for="column in columns" :key="column.key" class="text-center font-weight-bold text-caption py-1"
...
const productHeaders = [
  ...
  { title: 'Available Time', key: 'avail_hour', width: '160px', backgroundColor: '#FFFF66' },
